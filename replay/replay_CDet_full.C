...
void replay_CDet_full(UInt_t runnum, Long_t nevents=-1, Long_t firstevent=1, const char *fname_prefix="gep5", UInt_t firstsegment=0, UInt_t maxsegments=1, 
		Int_t maxstream=2, Int_t pedestalmode=0, Int_t cmplots=0, Int_t firststream=0, Int_t dogems=1, Int_t requiretrack=0, Int_t nontrackingmode=0)
{
    ...
    TString outfilename;
    TString outfile_prefix = "cdet_full";
    ...
    //analyzer->SetSummaryFile(Form("%s/%s_%d_stream%d_%d_seg%d_%d.log", prefix.Data(), outfile_prefix.Data(), runnum,
	  //  firststream, maxstream, firstsegment, lastsegment));
