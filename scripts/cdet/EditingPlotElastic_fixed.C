...
// Keep a 1:1 mapping between cached vectors and the underlying TTree entry
std::vector<Long64_t> vTreeEntry;
std::vector<char> vCDetPassed;
  ...
  while(reader.Next()){
    ...
    if (good_elastic){
      ...
      */
      ...
      bool good_CDet_X = true; // NOTE: no raw-hit x branch; don't index GoodX with RawElID loop
      ...
      if ( good_raw_event ) {
        ...
        //cout << " tdc = " << RawElLE[el]*TDC_calib_to_ns << endl;
          if ( (Int_t)RawElID[el] < 2688 ) {
            ...
            thisEvent_CDetX.push_back(NaN);
            thisEvent_CDetY.push_back(NaN);
            thisEvent_CDetZ.push_back(NaN);
    ...
    // Keep per-entry alignment (even if there are zero raw hits passing cuts)
...
//------------------------------------------------------- replace hist below
      ...
      // Keep per-entry alignment (even if there are zero good hits passing cuts)
      vTreeEntry.push_back(entry);
      vCDetPassed.push_back( (CDetPassedBoolCount >= 1) ? 1 : 0 );
