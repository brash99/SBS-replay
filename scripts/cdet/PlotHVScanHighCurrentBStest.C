...
// Function to get pixel for a given layer, submodule, side, and PMT 
std::vector<int> GetPixel(int layerNum, int submoduleNum, int sideNum, int pmtNum) {
  std::vector<int> thisPixel;
  ...
  while (std::getline(file, line)) {
      ...
      if (ss >> module >> comma >> side >> comma >> pmt >> comma >> pixel) {
          ...
          if (module == moduleNum && side == fileSide && pmt == (15 - pmtNum)) {
              if (sideNum == 0) {
                  thisPixel.push_back(pixel);
              } else {
                  thisPixel.push_back(17 - pixel);
              }
          }
      }
  }
...
void PlotHVScanHighCurrentBStest(Int_t RunNumber1=3562, Int_t nevents=50000, Int_t neventsr=50000,
