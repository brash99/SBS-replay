...
TH1F *hXDiffECalCDet1_min;
TH1F *hXDiffECalCDet2_min;
TH2F *hYECalCDet1;
TH2F *hYECalCDet2;
TH2F *hEECalCDet1;
TH2F *hEECalCDet2;
...
void EditingPlotElastic_minXdiff_timeMin(Int_t RunNumber1=5811, Int_t nevents=50000, Int_t neventsr=500000, Int_t elastic = 0, Int_t onlySegment = -1,
	...
	Int_t nruns=30, Int_t maxstream = 2, Int_t firstevent = 1)
{
  ...
  hXDiffECalCDet1_min = new TH1F("XDiffECalCDet1_min","XDiffECalCDet1_min (min |x-diff| hit per event)",NXDiffBins,XDiffLow,XDiffHigh);
  hXDiffECalCDet2_min = new TH1F("XDiffECalCDet2_min","XDiffECalCDet2_min (min |x-diff| hit per event)",NXDiffBins,XDiffLow,XDiffHigh);
...
// First pass through hits:  purpose is to get reference LE TDC Value for this event
    ...
    // --- NEW: per-event best (smallest |x-diff|) hit in each layer (0=L1, 1=L2)
    double bestAbsXDiff[2] = {1e99, 1e99};
    double bestXDiff[2]    = {0.0, 0.0};   // signed xdiff for the best hit
      ...
      if (goodhit_CDet_event) {
        ...
        if ( !check_bad(GoodElID[el], suppress_bad) ) {
          ...
          //GoodElTot[el] << " CDet X = " << GoodX[el] << " ECal X = " << ECalX << endl;
            ...
            // --- NEW: update per-layer best match based on smallest |x-diff|
            if (*ECalX != 0.00) {
...
//------------------------------------------------------- replace hist below
    ...
    // --- NEW: fill min-hit histograms once per event (per layer), using the best |x-diff| hit
    if (foundBest[0]) {
      hXECalCDet1_min->Fill(bestXCDet[0], bestXECalProj[0]);
      hXDiffECalCDet1_min->Fill(bestXDiff[0]);
    }
    if (foundBest[1]) {
      hXECalCDet2_min->Fill(bestXCDet[1], bestXECalProj[1]);
      hXDiffECalCDet2_min->Fill(bestXDiff[1]);
    }
  ...
  std::cout << "Size of GoodECalADCtime = " << v_GoodECalAdcTime.size() << std::endl;
...
void plotECalCDetTimeCompMin(double Width = 1, double diffMinCut = 76, double diffMaxCut = 106,
  ...
  // Unique histogram names so this can be called in the same ROOT session as plotECalCDetTimeComp()
  TH1D* hECalMinusCDetTime_min = new TH1D("hECalMinusCDetTime_min",
                                         "ECal-CDet Time (min |xDiff| hit per layer);Time Diff (ns);Counts",
                                         TDCBinNum, DiffMin, DiffMax);

  TH1D* hECalMinusCDetTimeNoCuts_min = new TH1D("hECalMinusCDetTimeNoCuts_min",
                                               "ECal-CDet Time (min |xDiff| hit per layer);Time Diff (ns);Counts",
                                               TDCBinNum, DiffMin, DiffMax);

  TH2D* h2ECalMinusCDetTime_min = new TH2D("h2ECalMinusCDetTime_min",
                                          "ECal-CDet Time vs CDet Time (min |xDiff| hit per layer);CDet 'Good' Time (ns);ECal-CDet Time (ns)",
                                          TDCBinNum, CDetMin, CDetMax, TDCBinNum, DiffMin, DiffMax);

  TH2D* h2ECalMinusCDetTot_min = new TH2D("h2ECalMinusCDetTot_min",
                                         "ECal-CDet Time vs CDet Tot (min |xDiff| hit per layer);CDet 'Good' Tot (ns);ECal-CDet Time (ns)",
                                         TDCBinNum, CDetTotMin, CDetTotMax, TDCBinNum, DiffMin, DiffMax);

  TH2D* hECalVsCDet_min = new TH2D("hECalVsCDet_min",
                                  "ECal Time vs CDet Time (min |xDiff| hit per layer);CDet LE Time (ns);ECal ADC Time (ns)",
                                  TDCBinNum, CDetMin, CDetMax, NADCBins, ECalMin, ECalMax);

  TH2D* h2ECalxVsCDetx_min = new TH2D("h2ECalxVsCDetx_min",
                                     "ECal Good x vs CDet Good x (min |xDiff| hit per layer);CDet Good x (m);ECal Good x (m)",
                                     600, -1.5, 1.5, 200, -1.5, 1.5);

  const size_t Nev = std::min(vGoodLe.size(), v_GoodECalAdcTime.size());

  int nFoundLayer1 = 0, nFoundLayer2 = 0;
  int nPassedCutsLayer1 = 0, nPassedCutsLayer2 = 0;

  for (size_t ev = 0; ev < Nev; ev++) {

    const double t_ECal = v_GoodECalAdcTime[ev];

    const size_t Nhits = std::min({ vGoodLe[ev].size(),
                                   vGoodTot[ev].size(),
                                   vCDetGoodX[ev].size(),
                                   vCDetGoodZ[ev].size(),
                                   vGoodLayer[ev].size() });
    ...
    int bestIdx[2] = {-1, -1};
    double bestAbs[2] = {1e99, 1e99};

    // Find min-|xDiff| hit index in each layer for this event
    for (size_t ihit = 0; ihit < Nhits; ++ihit) {
      const int layer = (int)vGoodLayer[ev][ihit];
      if (layer < 0 || layer > 1) continue;

      const double x_CDet = vCDetGoodX[ev][ihit];
      const double z_CDet = vCDetGoodZ[ev][ihit];
      const double x_ECal = v_GoodECalX[ev] * (z_CDet / ECal_dist);

      const double xdiff = x_CDet - x_ECal;
      const double axdiff = fabs(xdiff);

      if (axdiff < bestAbs[layer]) {
        bestAbs[layer] = axdiff;
        bestIdx[layer] = (int)ihit;
      }
    }

    // Fill using ONLY those best hits (one per layer, per event if present)
    for (int layer = 0; layer < 2; layer++) {
      const int idx = bestIdx[layer];
      if (idx < 0) continue;

      if (layer == 0) nFoundLayer1++;
      else nFoundLayer2++;

      const double t_CDet = vGoodLe[ev][idx];
      const double tot    = vGoodTot[ev][idx];
      const double t_diff = t_ECal - t_CDet;

      // "NoCuts" here means: the min-|xDiff| hit, regardless of the timing/tot cuts
      hECalMinusCDetTimeNoCuts_min->Fill(t_diff);

      if (t_CDet >= LeMin && t_CDet <= LeMax &&
          tot    >= TotMin && tot    <= TotMax &&
          t_diff >= diffMinCut && t_diff <= diffMaxCut) {

        const double x_CDet = vCDetGoodX[ev][idx];
        const double z_CDet = vCDetGoodZ[ev][idx];
        const double x_ECal = v_GoodECalX[ev] * (z_CDet / ECal_dist);

        hECalMinusCDetTime_min->Fill(t_diff);
        h2ECalMinusCDetTime_min->Fill(t_CDet, t_diff);
        h2ECalMinusCDetTot_min->Fill(tot, t_diff);
        hECalVsCDet_min->Fill(t_CDet, t_ECal);
        h2ECalxVsCDetx_min->Fill(x_CDet, x_ECal);

        if (layer == 0) nPassedCutsLayer1++;
        else nPassedCutsLayer2++;
      }
    }
  }

  std::cout << "plotECalCDetTimeCompMin(): Nev=" << Nev
            << "  found best hit: L1=" << nFoundLayer1 << "  L2=" << nFoundLayer2
            << "  passed cuts: L1=" << nPassedCutsLayer1 << "  L2=" << nPassedCutsLayer2
            << std::endl;

  // 1D overlay: cuts vs nocuts (for the min-|xDiff| hits)
  TCanvas *cTimeDiffMin = new TCanvas("cTimeDiffMin", "ECal ADCtime Minus CDet Good LE (min |xDiff| hit)", 900, 700);
  hECalMinusCDetTime_min->SetLineColor(kRed);
  hECalMinusCDetTimeNoCuts_min->SetLineColor(kBlue);
  hECalMinusCDetTimeNoCuts_min->Draw("HIST");
  hECalMinusCDetTime_min->Draw("SAME");

  auto legMin = new TLegend(0.7, 0.7, 0.9, 0.9);
  legMin->AddEntry(hECalMinusCDetTime_min, "Cuts", "l");
  legMin->AddEntry(hECalMinusCDetTimeNoCuts_min, "NoCuts", "l");
  legMin->Draw();

  // x correlation for min hit
  TCanvas *cXCompMin = new TCanvas("cXCompMin", "ECal x vs CDet x (min |xDiff| hit)", 900, 700);
  h2ECalxVsCDetx_min->Draw("COLZ");

  // 2D time comparisons for min hit
  TCanvas *c2DtimeCompsMin = new TCanvas("c2DtimeCompsMin", "ECal-CDet Time Comparisons (min |xDiff| hit)", 900, 700);
  c2DtimeCompsMin->Divide(1,3);

  c2DtimeCompsMin->cd(1);
  hECalVsCDet_min->Draw("COLZ");

  c2DtimeCompsMin->cd(2);
  h2ECalMinusCDetTime_min->Draw("COLZ");
...
auto plotXYECalCDet(){
   ...
   // 4x3 layout so we can show both cut-based and min-hit-based correlations + Xdiffs
   ...
   // ---------------- Row 2: ECal X/Y + Xdiff fits (cut-based) ----------------
   c8->cd(5);
   hXECal->Draw();
   ...
   c8->cd(8);
   {
     ...
     // Use x_min2/x_max2 here (avoid accidental reuse of x_min/x_max)
   ...
   // ---------------- Row 3: YECal vs YCDet + min-hit Xdiff ----------------
   c8->cd(9);
   hYECalCDet1->Draw();
   ...
   c8->cd(11);
   hXDiffECalCDet1_min->Draw();

   c8->cd(12);
   hXDiffECalCDet2_min->Draw();
