...
void PlotRawTDC2D(Int_t RunNumber=2232, Int_t nevents=50000, 
  ...
  for(Int_t event=0; event<NEventsAnalysis; event++){
	...
	if (TCDet::RawElTot[el] >= TotMin/TDC_calib_to_ns && TCDet::RawElTot[el] <= TotMax/TDC_calib_to_ns
	...
	if (TCDet::GoodElTot[el] >= TotMin/TDC_calib_to_ns && TCDet::GoodElTot[el] <= TotMax/TDC_calib_to_ns
