...
// ===================== Added by assistant: warp-enabled calibration =====================
#include <vector>
#include <limits>
#include <memory>
#include <cmath>

// Normal helpers and model CDF (names chosen to avoid collisions)
inline double __phi_std(double z){ return std::exp(-0.5*z*z)/std::sqrt(2.0*M_PI); }
inline double __Phi_std(double z){ return 0.5*(1.0 + std::erf(z/std::sqrt(2.0))); }
inline double __A_std(double z){ return z*__Phi_std(z) + __phi_std(z); }

inline double CDF_model_param(double x, double t0, double t1, double sigma,
                              double alpha=0.0, double beta=1.0)
{
  ...
  return alpha + beta * (sigma/W) * ( __A_std(z0) - __A_std(z1) );
}

// Empirical CDF builder (duplicate-safe name)
static TGraph* MakeEmpiricalCDF_Warp(const TH1* h, bool includeUF=false){
  if (!h) return nullptr;
  ...
  g->SetName("gEmpCDF_Warp");
  return g;
}

// Linear quantile from CDF graph (duplicate-safe name)
static double QuantileFromGraph_Warp(const TGraph* g, double u){
...
// Compute u10,u50,u90 anchors from summed histogram + fitted params
...
{
  std::unique_ptr<TGraph> gEmp(MakeEmpiricalCDF_Warp(hSum, includeUF));
  const double x10 = QuantileFromGraph_Warp(gEmp.get(), 0.10);
  const double x50 = QuantileFromGraph_Warp(gEmp.get(), 0.50);
  const double x90 = QuantileFromGraph_Warp(gEmp.get(), 0.90);
  auto F = [&](double x){ return CDF_model_param(x, t0_fit,t1_fit,sigma_fit,alpha_fit,beta_fit); };
  u10 = F(x10); u50 = F(x50); u90 = F(x90);
  // enforce monotone and clamp
  if (!std::isfinite(u10)) u10 = 0.1;
  if (!std::isfinite(u50)) u50 = 0.5;
  if (!std::isfinite(u90)) u90 = 0.9;
  if (u10 < 0) u10 = 0; if (u10 > 1) u10 = 1;
  if (u50 < u10) u50 = u10; if (u50 > 1) u50 = 1;
  if (u90 < u50) u90 = u50; if (u90 > 1) u90 = 1;
}
...
// Calibrate a single raw value using warp, mapped to nominal [t0_nom,t1_nom]
...
{
  double u_raw = CDF_model_param(x_raw, t0_fit, t1_fit, sigma_fit, alpha_fit, beta_fit);
  double u = (u_raw - alpha_fit) / beta_fit;
  if (clamp_u) { if (u<0) u=0; else if (u>1) u=1; }
  const double u_corr = g_warp(u, u10, u50, u90);
  return t0_nom + (t1_nom - t0_nom) * u_corr;
}

// Convenience: make uniform reference on [t0,t1]
static TH1D* MakeUniformRef_Warp(int nbins, double t0, double t1, double totalCounts){
  TH1D* href = new TH1D("href_uniform_warp","Uniform reference;Calibrated TDC;Counts",
                        nbins, t0, t1);
...
// Test driver: same as your TestParamCalibration but with warp toggle
TCanvas* TestParamCalibration_LSMB_Warp(int layer, int side, int module, int bar_local,
                                   const char* paramCsv = "tdc_cdf_params.csv",
                                   int rebin=1, bool includeUF=false,
                                   bool useWarp=true,
                                   const char* saveAs=nullptr)
{
  // Expect globals: loader gParamsLSMB, MakeSumRawLe_LSMB or similar
  // Try to find existing sum-maker; fallback to MakeSumRawLe if present
  // We'll declare weak externs to avoid compile errors if both exist
  extern TH1D* MakeSumRawLe_LSMB(int,int,int,int,int);
  TH1D* hSum = nullptr;
  if (MakeSumRawLe_LSMB) hSum = MakeSumRawLe_LSMB(layer,side,module,bar_local,rebin);
  if (!hSum){
    // try MakeSumRawLe(layer,side,module,bar_local,rebin) if available
    extern TH1D* MakeSumRawLe(int,int,int,int,int);
    if (MakeSumRawLe) hSum = MakeSumRawLe(layer,side,module,bar_local,rebin);
  }
  if (!hSum){ std::cerr<<"[TestParamCalibration_LSMB_Warp] missing hSum\n"; return nullptr; }

  // Load params from CSV (reuse user's loader if present)
  // We'll implement a minimal local loader for this call
  struct CdfParams { double t0, t1, sigma, alpha, beta; bool ok; };
  CdfParams P{10.0,11.127,0.05,0.0,1.0,false};

  // parse CSV one pass to find this (L,S,M,B)
  std::ifstream is(paramCsv);
  if (!is.good()){ std::cerr<<"[TestParamCalibration_LSMB_Warp] cannot open "<<paramCsv<<"\n"; return nullptr; }
  ...
  while (std::getline(is,line)){
    ...
    if (L==layer && S==side && M==module && B==bar_local){
      P = {t0,t1,sig,a,b, okint==1};
      break;
    }
  }
  if (!P.ok){ std::cerr<<"[TestParamCalibration_LSMB_Warp] params not ok\n"; }

  // Build CDF graph for overlay
  std::unique_ptr<TGraph> gEmp(MakeEmpiricalCDF_Warp(hSum, includeUF));
  TF1* fModel = new TF1("fModel_param",
    [P](double* xx, double*){ return CDF_model_param(xx[0], P.t0,P.t1,P.sigma,P.alpha,P.beta); },
    hSum->GetXaxis()->GetXmin(), hSum->GetXaxis()->GetXmax(), 0);
  fModel->SetLineColor(kRed+1); fModel->SetLineStyle(2);

  // Compute warp anchors
  double u10=0.1,u50=0.5,u90=0.9;
  ComputeWarpAnchors_u(hSum, P.t0,P.t1,P.sigma,P.alpha,P.beta, u10,u50,u90, includeUF);

  // Build calibrated histogram by per-bin redistribution using (warped) mapping
  const int nbx = hSum->GetNbinsX();
  TH1D* hCal = (TH1D*)hSum->Clone("hCal_warp"); hCal->Reset("ICES"); hCal->SetDirectory(nullptr);
  hCal->GetXaxis()->Set(nbx, 10.0, 11.127);

  auto overlap_len = [](double a0,double a1,double b0,double b1){
  ...
  for (int b=1;b<=nbx;++b){
    ...
    auto mapOne = [&](double x)->double{
      double u_raw = CDF_model_param(x, P.t0,P.t1,P.sigma,P.alpha,P.beta);
      double u = (u_raw - P.alpha)/P.beta; if (u<0) u=0; else if (u>1) u=1;
      if (useWarp) u = g_warp(u, u10,u50,u90);
      return 10.0 + (11.127-10.0)*u;
    };
    double y0 = mapOne(x0), y1 = mapOne(x1);
  ...
  TH1D* hRef = MakeUniformRef_Warp(nbx, 10.0, 11.127, hCal->Integral(1,nbx));

  // Draw
  TCanvas* c = new TCanvas(Form("c_test_warp_L%dS%dM%dB%02d",layer,side,module,bar_local),
                           "Parametric calibration with warp", 1200, 900);
  ...
  TH1D* frame = (TH1D*)hSum->Clone("frameCDFw"); frame->Reset("ICES"); frame->SetDirectory(nullptr);
  frame->SetTitle("Empirical CDF vs model;TDC;CDF");
  frame->SetMinimum(0.0); frame->SetMaximum(1.05);
  frame->Draw();
  if (gEmp) { gEmp->SetMarkerStyle(20); gEmp->SetMarkerSize(0.6); gEmp->SetLineWidth(2); gEmp->Draw("LP SAME"); }
  fModel->Draw("SAME");
  ...
  double total = hCal->Integral(1,nbx);
  for (int b=1;b<=nbx;++b){ cum2 += hCal->GetBinContent(b); Y.push_back(hCal->GetXaxis()->GetBinUpEdge(b)); V.push_back(total>0?cum2/total:0.0); }
  TGraph* gCDFcal = new TGraph((int)Y.size(), Y.data(), V.data());
  gCDFcal->SetLineWidth(2); gCDFcal->SetLineColor(kBlue+2);
  TH1D* fr2 = (TH1D*)frame->Clone("fr2w"); fr2->SetTitle("Calibrated CDF vs ideal;Calibrated TDC;CDF");
  fr2->Draw(); gCDFcal->Draw("LP SAME");
  TLine* ideal = new TLine(10.0,0, 11.127,1); ideal->SetLineColor(kRed+1); ideal->SetLineStyle(2); ideal->Draw("same");
