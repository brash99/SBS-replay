...
void PlotHVScan(Int_t RunNumber1=3311, Int_t nevents=50000, Int_t neventsr=50000,
  ...
  hnhits1 = new TH1F("hnhits1","hnhits1",50,1,51);
  hngoodhits1 = new TH1F("hngoothits1","hngoodhits1",10,1,11);
  hngoodTDChits1 = new TH1F("hngoodTDChits1","hngoodTDChits1",10,1,11);

  hnhits2 = new TH1F("hnhits2","hnhits2",50,1,51);
  hngoodhits2 = new TH1F("hngoothits2","hngoodhits2",10,1,11);
  hngoodTDChits2 = new TH1F("hngoodTDChits2","hngoodTDChits2",10,1,11);
  ...
  hnpaddles = new TH1F("hnpaddles","hnpaddles",50,1,51);
  hngoodpaddles = new TH1F("hngoodpaddles","hnTDCpaddles",10,1,11);
  hngoodTDCpaddles = new TH1F("hngoodTDCpaddles","hngoodTDCpaddles",10,1,11);
  ...
  for(Int_t event=0; event<NEventsAnalysis; event++){
	...
	if (TCDet::GoodElLE[el] >= LeMin/TDC_calib_to_ns && TCDet::GoodElLE[el] <= LeMax/TDC_calib_to_ns 
		...
		&& TCDet::GoodECalY > -1.2 && TCDet::GoodECalY < 1.2  
	) {
	...
	if (TCDet::GoodElLE[el] >= LeMin/TDC_calib_to_ns && TCDet::GoodElLE[el] <= LeMax/TDC_calib_to_ns 
		&& TCDet::GoodElTot[el] >= TotMin/TDC_calib_to_ns && TCDet::GoodElTot[el] <= TotMax/TDC_calib_to_ns 
		&& TCDet::GoodX[el] < xcut && TCDet::TDCmult[el] < TDCmult_cut 
		&& ngoodhitsc1 >= nhitcutlow1  && ngoodhitsc2 >= nhitcutlow2 
		&& ngoodhitsc1 <= nhitcuthigh1 && ngoodhitsc2 <= nhitcuthigh2 
		&& (TCDet::GoodX[el]-(TCDet::GoodECalX)-XOffset) <= XDiffCut && (TCDet::GoodX[el]-(TCDet::GoodECalX)-XOffset) >= -1.0*XDiffCut
		&& TCDet::GoodECalX > -1.5 && TCDet::GoodECalX < 1.5  
		&& TCDet::GoodECalY > -1.2 && TCDet::GoodECalY < 1.2  
	) {
  ...
  for (Int_t b=0; b<NumCDetPaddles; b++) {
	...
	if (hRawLe[b]->GetEntries() > EventCounter/NumCDetPaddles*2*20) {
		...
		"   bar = " << myhotbar << "   paddle_PMT = " << myhotpaddle << "   Entries = " << hRawLe[b]->GetEntries() << std::endl;
	}
  }
