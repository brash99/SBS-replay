...
static const int nTdc = NumCDetPaddles+NumRefPaddles; //2880

static const int NumSidesTotal = NumSides*NumLayers;
static const int NumCDetPaddlesPerSide = NumCDetPaddles/NumSidesTotal; //672
static const int NumLogicalPaddlesPerSide = nTdc/NumSidesTotal; //720

static const int nBarsADC = 0;
static const double ADCCUT = 150.;   //100.0

static const double ecal_dist = 8.0;
static const double cdet_dist_offset = 0.0;
...
const int num_bad = 111;

const int bad_channels[] = {
	45,58,61,
	64,65,66,67,68,69,70,71,72,73,74,75,76,77,78,79,
	80,81,82,83,84,85,86,87,88,89,90,91,92,93,94,95,
	 417,1161,1286,1291,1472,
	2124,2140,2404,2406,2414,
	2420,2422,
	49,51,56,136,169,174,
	183,184,192,208,209,210,
	211,214,215,298,406,408,
	411,413,490,491,693,694,
	918,1280,1281,1283,1287,1288,
	1295,2405,2430,2629,2662,2666,
	1669,1696,1896,
	48,62,168,205,219,302,407,424,489,533,
	816,820,933,949,1189,1191,1199,1255,1284,1292,
	1302,2602,2630,2661,2665
};
...
void PlotHVScanHighCurrentKin1(Int_t RunNumber1=3562, Int_t nevents=50000, Int_t neventsr=50000,
  ...
  for(Int_t event=0; event<NEventsAnalysis; event++){
	...
	//	TCDet::RawElTot[el] << " CDet X = " << TCDet::GoodX[el] << " ECal X = " << TCDet::GoodECalX << endl;
	  ...
	  if ( !check_bad(TCDet::RawElID[el],suppress_bad) ) {
	   ...
	   if ( (Int_t)TCDet::RawElID[el]%NumSidesTotal < NumCDetPaddlesPerSide ) {
	    ...
	    if ((Int_t)TCDet::RawElID[el]%NumSidesTotal >= NumCDetPaddlesPerSide) cout << " Ref  ID = " << (Int_t)TCDet::RawElID[el] << "    TDC = " << TCDet::RawElLE[el]*TDC_calib_to_ns << endl;
	...
	bool good_ecal_diff = (TCDet::GoodX[el]-(TCDet::GoodECalX*(TCDet::GoodZ[el]-cdet_dist_offset)/ecal_dist)-XOffset) <= XDiffCut && 
			(TCDet::GoodX[el]-(TCDet::GoodECalX*(TCDet::GoodZ[el]-cdet_dist_offset)/ecal_dist)-XOffset) >= -1.0*XDiffCut;

	bool good_CDet_event = good_ecal_reconstruction && good_ecal_diff && good_le_time && good_tot && good_hit_mult && good_cdet_X;
	...
	bool goodhit_ecal_diff = (TCDet::GoodX[el]-(TCDet::GoodECalX*(TCDet::GoodZ[el]-cdet_dist_offset)/ecal_dist)-XOffset) <= XDiffCut && 
			(TCDet::GoodX[el]-(TCDet::GoodECalX*(TCDet::GoodZ[el]-cdet_dist_offset)/ecal_dist)-XOffset) >= -1.0*XDiffCut;


	bool goodhit_CDet_event = goodhit_ecal_reconstruction && goodhit_ecal_diff && goodhit_le_time && goodhit_tot 
  ...
  for (Int_t b=0; b<NumCDetPaddles; b++) {
	...
	if (hRawLe[b]->GetEntries() > EventCounter/NumCDetPaddles*2*2) {
...
void plotPMTRates(Int_t mymodule=1, Int_t mylayer=1, Int_t choice = 1){
  ...
  Double_t rate_levelu = 111; // Expect 600 for 50000 events @ 5uA
  Double_t rate_levell = 37; // Expect 600 for 50000 events @ 5uA
