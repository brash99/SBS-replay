...
namespace TCDet {
  ...
  Double_t ngoodTDChits_paddles[nTdc];
...
void PlotRawTDC2DCut2Runs(Int_t RunNumber1=2845, Int_t RunNumber2=2846, Int_t nevents=60000, Int_t neventsr=300000, 
  ...
  if(!T) { 
    ...
    subfile = TString::Format("_%d_%d",RunNumber2,neventsr);
    sInFile = REPLAYED_DIR + "/" + InFilePrefix + subfile + ".root";
    cout << "Input ROOT file = " << sInFile << endl;
    cout << "Adding " << sInFile << endl;
    T->Add(sInFile);
    cout << "Adding " << nruns << " files ... " << endl;
    for (Int_t i=1; i<=nruns; i++) {
    	subfile = TString::Format("_%d_%d_%d",RunNumber2,neventsr,i);
    	//subfile = TString::Format("_%d_1000000_%d",RunNumber2,i);
    	sInFile = REPLAYED_DIR + "/" + InFilePrefix + subfile + ".root";
    	cout << "Input ROOT file = " << sInFile << endl;
    	cout << "Adding " << sInFile << endl;
    	T->Add(sInFile);
  ...
  for(Int_t event=0; event<NEventsAnalysis; event++){
	...
	if (TCDet::GoodElLE[el] >= LeMin/TDC_calib_to_ns && TCDet::GoodElLE[el] <= LeMax/TDC_calib_to_ns 
		...
		&& (TCDet::GoodX[el]-TCDet::GoodECalX) <= XDiffCut && (TCDet::GoodX[el]-TCDet::GoodECalX) >= -1.0*XDiffCut  
	) {
