...
inline void ConfigureCalibrationStage(int stage) {
  gUseECalTimeCorr = StageUsesECalCorrection(stage);
  gUseTimeWalkCorr = StageUsesTimeWalkCorrection(stage);

  if (StageUsesBarOffsets(stage)) {
  ...
  std::cout << "[CDet] Calibration stage " << stage << ": "
            ...
            << "        applyBarOffsets=" << (StageUsesBarOffsets(stage) ? "true" : "false")
...
void PlotElastic_Calibration_Master_stageflag(Int_t RunNumber1=5811, Int_t nevents=50000, Int_t calibStage = 7, Int_t elastic = 0, Int_t minSeg = -1, Int_t maxSeg = -1,
...
void plotGoodLeVsTotByLayer(double leMin = 0.0, double leMax = 60.0,
    ...
    if (fitTimeWalk) {
      ...
      if (fitMax > fitMin) {
        ...
        std::cout << "\n[plotGoodLeVsTotByLayer] Layer 1 time-walk fit:\n"
                  ...
                  << "  p0 = " << p0 << " +/- " << e0 << " ns\n"
                  << "  p1 = " << p1 << " +/- " << e1 << " ns*sqrt(ns)\n"
                  << "  To apply in the main macro, set gTimeWalkP1_L1 = " << p1 << ";\n";
    ...
    if (fitTimeWalk) {
      ...
      if (fitMax > fitMin) {
        ...
        std::cout << "\n[plotGoodLeVsTotByLayer] Layer 2 time-walk fit:\n"
                  ...
                  << "  p0 = " << p0 << " +/- " << e0 << " ns\n"
                  << "  p1 = " << p1 << " +/- " << e1 << " ns*sqrt(ns)\n"
                  << "  To apply in the main macro, set gTimeWalkP1_L2 = " << p1 << ";\n";
...
TCanvas *plotAllTDC(bool overwrite = false, double width = 1, double binLow = 0, double binHigh = 60){
  ...
  if (shouldWriteOffsets) {
    ...
    if (fout) {
      fout << "# CDet bar time offsets (ns) to ADD to LE/TE times\n";
      fout << "# total_offset = old_offset + residual_offset\n";
      fout << "# residual_offset = mean_all - mean_bar from current pass\n";
      fout << "# Columns: bar(1..168)  total_offset_ns  residual_offset_ns  entries\n";
      ...
      for (int bar = 0; bar < NumPMTs; ++bar) {
        ...
        fout << (bar+1) << " "
             << vBarTotalOffset[bar] << " "
             << vBarResidualOffset[bar] << " "
             << ent << "\n";
      }

      if (overwrite && gBarToffsetLoaded) {
        std::cout << "[CDet] Overwrote offset file '" << gBarToffsetFile
                  << "' with UPDATED total offsets = old + residual\n";
      } else {
        std::cout << "[CDet] Wrote offset file '" << gBarToffsetFile
                  << "' using offsets from current pass\n";
      }
    } else {
